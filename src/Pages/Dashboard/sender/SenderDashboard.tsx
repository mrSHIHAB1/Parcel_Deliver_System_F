          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredParcels.map((p) => (
                <tr
                  key={p._id}
                  className="hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer block md:table-row mb-4 md:mb-0"
                >

                  <td className="px-4 py-2 text-gray-800 dark:text-gray-200 block md:table-cell">
                    <span className="font-semibold md:hidden">Tracking ID: </span>
                    {p.trackingId}
                  </td>
                  <td className="px-4 py-2 text-gray-800 dark:text-gray-200 block md:table-cell">
                    <span className="font-semibold md:hidden">Receiver: </span>
                    {p.receiver}
                  </td>
                  <td className="px-4 py-2 text-gray-800 dark:text-gray-200 block md:table-cell">
                  ...
                  <td className="px-4 py-2 text-gray-800 dark:text-gray-200 block md:table-cell">
                    <span className="font-semibold md:hidden">Weight: </span>
                    {p.weight} kg
                  </td>
                  <td className="px-4 py-2 block md:table-cell">
                    <span className="font-semibold md:hidden">Status: </span>
                    <span
                      className={`px-2 py-1 rounded-full text-white font-semibold ${p.status === "Delivered"
                          ? "bg-green-500"
                          : p.status === "In Transit"
                            ? "bg-yellow-500"
                            : p.status === "Pending"
                  ...
                  <td className="px-4 py-2 text-gray-800 dark:text-gray-200 block md:table-cell">
                    <span className="font-semibold md:hidden">Fee: </span>
                    {p.fee}
                  </td>
                </tr>
              ))}
              ...
              {filteredParcels.length === 0 && (
                <tr className="block md:table-row">
                  <td
                    colSpan={6}
                    className="text-center py-4 text-gray-500 dark:text-gray-400 block md:table-cell"
                  >
                    No parcels found.
                  </td>
                </tr>
